package edu.tec.mensajespositivos;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textView.setText(mensajes[0]);

        editText = findViewById(R.id.editText);
    }

    public void dia(View view){
        ConstraintLayout mConstraintLayout = findViewById(R.id.constraintLayoutID);
        mConstraintLayout.setBackgroundResource(R.drawable.day);
    }

    public void noche(View view){
        ConstraintLayout mConstraintLayout = findViewById(R.id.constraintLayoutID);
        mConstraintLayout.setBackgroundResource(R.drawable.night);
    }
    ...
    public void adelante(View view){
        if(position >= 0 && position < mensajes.length-1){
            position += 1;
            textView.setText(mensajes[position]);
        }else{
            position = 0;
            textView.setText(mensajes[position]);
        }
    ...
    public void anterior(View view){
        if(position > 0 && position < mensajes.length){
            position -= 1;
            textView.setText(mensajes[position]);
        }else if(position <= 0){
            position = mensajes.length-1;
            textView.setText(mensajes[position]);
        }else{
            position = 0;
            textView.setText(mensajes[position]);
    ...
    public void selectNum(View view){
        newPos1 = editText.getText().toString();
        newPos2 = Integer.parseInt(newPos1) - 1;
        if(newPos2 <= 0 || newPos2 >= mensajes.length){
            Toast.makeText(this,"Ingresa una posición válida",Toast.LENGTH_LONG).show();
            Log.i(TAG, "Se ingresó una posición inválida");
        }else{
            position = newPos2;
            textView.setText(mensajes[position]);
        }
